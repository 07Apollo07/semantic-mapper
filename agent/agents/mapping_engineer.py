...
from .agents_utils import SemanticMappingState, MappingOutput
...
def create_mapping_engineer(retriever, model_name="gpt-4o", api_key=None, base_url=None):
    ...
    def engineer_node(state: SemanticMappingState):
        ...
        print(f"[Engineer Debug] Fetched global/mapping instructions.")
...
{state.get('fsdm_lineage_intent', 'No lineage context provided.')}
</fsdm_lineage_intelligence>
...
3. **Naming Precision:** Be exact with table and column names. Use double-quotes if names contain spaces or special characters.
4. **Join Awareness:** If the lineage intent implies multiple tables, reflect the join logic in your reasoning but provide the specific column expression.
5. **Tool Termination:** YOU MUST CALL `MappingOutput` to submit your final SQL transformation.

### Process Flow
1. **Intelligence Analysis:** Use `<fsdm_lineage_intelligence>` and `<retrieved_documentation>` to determine the correct filtering and transformation logic.
...
4. **Submission:** Call `MappingOutput` with the transformation type, logic, and a detailed reasoning.

### Execution Strategy
- Decide the appropriate transformation type (1:1, Join, Aggregation, Case expression).
- Ensure the logic is robust and adheres to all project rules.
- **YOU MUST CALL `MappingOutput` to finish your task.**
"""
