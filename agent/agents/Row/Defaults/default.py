"""Default executor implementing the original two‑step flow.

This class was previously named ``AgentExecutor`` and contains the full logic
for:
* Initialising the FSDM Detective and Mapping Engineer agents.
* ``process_row`` – runs both phases.
* ``process_fsdm_only`` – runs only the detective.
* ``process_mapping_only`` – runs only the engineer.
* ``process_mapping_custom`` – runs the custom mapping agent.
* ``process_table_group`` – batch processing for a whole table.

The wrapper in ``agent/agents/executor.py`` delegates to this class when the
selected ``agent_name`` is ``DEFAULT``.
"""

import pandas as pd
from typing import Dict, Any, List, Optional
from agent.agents.Row.Defaults.fsdm_detective import create_fsdm_detective
# from agent.agents.mapping_engineer import create_mapping_engineer
from agent.agents.Row.Defaults.mapping_oneshot import create_mapping_oneshot
...
from agent.agents.Table.mapping_table_group_agent import create_mapping_table_group
...
class DefaultExecutor:
    ...
    def process_row(self, row_data: Dict[str, Any], row_idx: int, feedback: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Phase 1: FSDM Lineage Discovery (Detective)
        ...
        # Phase 2: SQL Engineering (Engineer)
    ...
    def process_mapping_only(self, row_data: Dict[str, Any], row_idx: int, feedback: Optional[str] = None) -> Dict[str, Any]:
        ...
        mapping_res = {"row_idx": row_idx, "mapping_status": "Pending"}
    ...
    def process_table_group(self, table_name: str, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        inputs = {"row_data": rows, "project_name": self.state.current_project, "feedback": None}
