...
def create_mapping_table_group(model_name="gpt-4o", api_key=None, base_url=None, log_callback=None):
    """Batch mapping agent for an entire table.

    Expects ``row_data`` to be a list of row dictionaries.
    ...
    def init_node(state: MappingState):
        ...
        # Convert list of dicts to DataFrame, drop all‑null columns
        ...
        rows_csv = df.apply(lambda r: ','.join([str(v) if pd.notna(v) else '' for v in r]), axis=1).tolist()
        rows_str = "\n".join(rows_csv)
...
Goal: Generate a correct SQL mapping for an entire target table based on many row specifications.
"""
        user_content = f"""
...
"""
    ...
    def generate_node(state: MappingState):
        ...
        print(f"\n--- [Table Group Mapping: System Prompt] ---")
        messages[0].pretty_print()
        print(f"\n--- [Table Group Mapping: User Content] ---")
